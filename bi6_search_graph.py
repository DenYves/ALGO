...
def bfs6_state_graph_bidirectional(adj, dist, s_a, t_a, s_b, t_b, D, T):
    ...
    while q_s or q_t:
        ...
        if side == 0:
            ...
            for nx in adj[x]:
                for ny in adj[y]:
                    # distance constraint between A and B
                    d_xy = dist[nx][ny]
                    ...
                    # heuristic from child (nx, ny) to (t_a, t_b)
                    dx_ta = dist[nx][t_a]
                    dy_tb = dist[ny][t_b]
                    if dx_ta == -1 or dy_tb == -1:
                        # cannot reach both targets at all → useless state
                        continue
                    h_child = max(dx_ta, dy_tb)
                    if d_new + h_child >= best_total:
                        # even best-case continuation can't beat current best
                        continue
            ...
            for nx in adj[x]:
                for ny in adj[y]:
                    ...
                    # heuristic from child (nx, ny) to (s_a, s_b)
                    dx_sa = dist[nx][s_a]
                    dy_sb = dist[ny][s_b]
                    if dx_sa == -1 or dy_sb == -1:
                        continue
                    h_child = max(dx_sa, dy_sb)
                    if d_new + h_child >= best_total:
                        continue
